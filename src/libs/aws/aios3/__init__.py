...
def guess_content_type(file_obj: IO[bytes]) -> str:
...
class Bucket:
    ...
    async def upload_fileobj(self, fp: IO[bytes], key: str) -> None:
        await self.bucket.upload_fileobj(
            fp, key, {"ACL": "public-read", "ContentType": guess_content_type(fp)}
        )

    async def download_fileobj(self, fp: IO[bytes], key: str) -> None:
        await self.bucket.download_fileobj(key, fp)

    async def delete_objects(self, key: str) -> None:
